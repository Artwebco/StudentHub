                                ...
                                <tr class="border-t">
                                    ...
                                    <td class="p-4 text-right space-x-2">
                                        <button type="button" wire:click="edit({{ $student->id }})"
                                            title="{{ __('admin.students.edit') }}" aria-label="{{ __('admin.students.edit') }}"
                                        ...
                                        <button type="button"
                                            onclick="confirmDelete({{ $student->id }}, @js(trim(($student->first_name ?? '') . ' ' . ($student->last_name ?? ''))))"
                                            title="{{ __('admin.students.delete') }}" aria-label="{{ __('admin.students.delete') }}"
                                            class="inline-flex items-center gap-2 px-3 py-1 text-sm font-medium rounded-md text-red-600 md:bg-red-50 md:hover:bg-red-100 md:hover:text-red-800 transition focus:outline-none">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline" fill="none"
                                                viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div x-show="open" x-transition:enter="transform transition ease-out duration-300"
                ...
                <div class="px-8 py-8 space-y-4 overflow-y-auto flex-1">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label
                                class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{{ __('admin.students.label_first_name') }}
                                (МК)</label>
                            <input type="text" wire:model="first_name_mk" placeholder="Име (МК)"
                                class="w-full h-11 border-gray-200 rounded-xl shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                            <x-input-error :messages="$errors->get('first_name_mk')" class="mt-1" />
                        </div>
                        <div>
                            <label
                                class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{{ __('admin.students.label_last_name') }}
                                (МК)</label>
                            <input type="text" wire:model="last_name_mk" placeholder="Презиме (МК)"
                                class="w-full h-11 border-gray-200 rounded-xl shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                            <x-input-error :messages="$errors->get('last_name_mk')" class="mt-1" />
                        </div>
                    </div>
