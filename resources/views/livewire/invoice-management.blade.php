<div>
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 border-b pb-4 gap-4">
        <div>
            <h2 class="text-xl font-bold text-gray-700">Управување со фактури</h2>
            <p class="text-md text-gray-600">Преглед и управување со фактури</p>
        </div>
        ...
        <div class="bg-gray-50 p-4 rounded-xl border border-gray-200">
            ...
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                ...
                <div class="relative w-full sm:w-80 order-1">
                    ...
                    <input type="text" wire:model.live="search" placeholder="Пребарај клиент или бр. фактура..."
                ...
                <div class="order-2 grid grid-cols-[1fr_1fr_auto] gap-2 w-full sm:flex sm:w-auto sm:items-center">
                    ...
                    <select wire:model.live="filter_status"
                        ...
                        <option value="cancelled">Поништени</option>
                        <option value="sent">Испратени</option>
                        <option value="unsent">Непратени</option>
                    </select>
                    ...
                    <select wire:model.live="filter_type"
                        ...
                        <option value="student">Настава</option>
                        <option value="service">Услуги</option>
                    </select>

                    {{-- Ресетирај --}}
                    <button wire:click="resetFilters" title="Исчисти филтри"
            ...
            <table class="w-full border-collapse bg-white">
                <thead class=" bg-gray-50 text-left font-semibold text-sm text-gray-900">
                    <tr>
                        ...
                        <th class="px-2 py-3">Ученик / Клиент</th>
                        <th class="px-2 py-3">Износ</th>
                        <th class="px-2 py-3">Реализација</th>
                        <th class="px-2 py-3">Статус</th>
                        <th class="px-2 py-3">Опис на услуга</th>
                        ...
                        <tr
                            ...
                            <td class="px-2 py-2 whitespace-nowrap text-md font-medium text-gray-600">
                                @if($invoice->student)
                                    ...
                                    <div class="text-xs text-gray-400 font-medium">Настава</div>
                                @else
                                    <div class=" text-blue-700">{{ $invoice->custom_client_name }}</div>
                                    <div class="text-xs text-gray-400 font-medium">Услуга</div>
                                @endif
                            </td>
                                ...
                                @if($invoice->is_advance && $invoice->student_id)
                                    <div class="flex flex-col items-start">
                                        <span class="font-semibold text-base text-blue-900">
                                            {{ $invoice->realized_lessons }}<span class="text-gray-400"> од
                                            </span>{{ $invoice->expected_lessons }}<span class="text-gray-400"> часа</span>
                                        </span>
                                        @if($invoice->progress_state === 'under')
                                            <span
                                                ...
                                                Недостигаат {{ $invoice->remaining_lessons }}
                                            </span>
                                        @elseif($invoice->progress_state === 'over')
                                            <span
                                                ...
                                                Надминато {{ abs($invoice->remaining_lessons) }}
                                            </span>
                            ...
                            <td class="px-2 py-2 whitespace-nowrap text-md font-medium">
                                @if($invoice->status === 'paid')
                                    <span
                                        class="inline-flex items-center gap-1.5 py-1 px-3 rounded-full text-xs bg-green-100 text-green-600 border border-green-200">Платено</span>
                                @elseif($invoice->status === 'cancelled')
                                    <span
                                        class="inline-flex items-center gap-1.5 py-1 px-3 rounded-full text-xs bg-red-100 text-red-700 border border-red-200">Поништено</span>
                                    @if($invoice->cancelled_reason)
                                        <div class="mt-1.5 text-[11px] text-red-700/80 max-w-[240px] truncate flex items-center gap-1"
                                            ...
                                            Причина: {{ \Illuminate\Support\Str::limit($invoice->cancelled_reason, 38) }}
                                ...
                                @else
                                    <span class="inline-flex items-center gap-1.5 py-1 px-3 rounded-full text-xs border"
                                        style="background-color:#fef3c7;color:#92400e;border-color:#fcd34d;">Неплатено</span>
                                @endif
                                ...
                                @if($invoice->email_sent_at)
                                    <div class="mt-2 text-[11px] leading-4 max-w-xs" style="color:#2563eb;">
                                        <span class="inline-flex items-center gap-1 py-0.5 px-2 rounded-full border"
                                            ...
                                            Испратена{{ (int) $invoice->email_sent_count > 1 ? ' x' . (int) $invoice->email_sent_count : '' }}
                            ...
                            <td class="px-2 py-2 text-sm text-gray-500 max-w-xs truncate">
                                ...
                                @elseif($invoice->is_advance)
                                    ...
                                    <span class="italic">Аванс за настава</span>
                                @else
                                            ...
                                            <template x-teleport="body">
                                                <div x-show="open" @click.outside="open = false" :style="menuStyle"
                                                    ...
                                                    <button wire:click="togglePaid({{ $invoice->id }})" @click="open = false"
                                                        ...
                                                        {{ $invoice->status === 'paid' ? 'Означи како неплатена' : 'Означи како платена' }}
                                                    </button>
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="text-gray-400">
                                    <p class="text-lg font-semibold">Не се пронајдени резултати</p>
                                    <p class="text-sm">Обидете се со поинакво пребарување.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                        ...
                        <td colspan="7" class="px-3 py-2">
                            <div class="flex items-center justify-end gap-6 text-sm leading-none">
                                <div class="flex items-center gap-2">
                                    <span class="font-semibold text-blue-800 uppercase text-[11px]">Вкупно:</span>
                                    <span
                                ...
                                @if(($totalUnpaidAmount ?? 0) > 0)
                                    <div
                                        ...
                                        <span class="font-semibold text-amber-800 uppercase text-[11px]">Неплатено:</span>
                                        <span
            ...
            <div x-data="{ open: false }" x-init="setTimeout(() => open = true, 10)"
                ...
                <div x-show="open" x-transition:enter="animate-swal-show" x-transition:leave="animate-swal-hide"
                    ...
                    <div class="px-5 py-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
                        <div>
                            <h3 class="text-xl font-bold text-gray-900 tracking-tight">Нова фактура</h3>
                        </div>
                    ...
                    <div class="px-5 py-5 space-y-4">
                        ...
                        <div>
                            <label class="block text-[11px] text-gray-400 uppercase tracking-widest mb-1 font-bold">Тип на
                                услуга</label>
                            <div class="grid grid-cols-2 gap-2">
                                <label
                                    ...
                                    <span class="text-xs font-semibold">📚 НАСТАВА</span>
                                </label>
                                <label
                                    ...
                                    <span class="text-xs font-semibold">💻 УСЛУГИ</span>
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            @if($invoice_type === 'student')
                                ...
                                @if($is_advance)
                                    <div class="mb-2 w-full relative" x-data="{ open: false }" x-on:click.outside="open = false">
                                        <label class="block text-xs text-gray-500 uppercase tracking-wider mb-1 font-bold">Избери
                                            ученик</label>
                                        <div @click="open = !open"
                                    ...
                                    <div class="grid grid-cols-12 gap-2 animate-fadeIn">
                                        <div class="col-span-6">
                                            <label class="block text-xs text-gray-500 uppercase tracking-wider mb-1 font-bold">Тип
                                                на час</label>
                                            <select wire:model="lesson_type_id"
                                                ...
                                                @foreach($lessonTemplates as $template)
                                                    <option value="{{ $template->id }}">{{ $template->name }}
                                                        ({{ $template->default_price }} ден.)</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-span-3">
                                            <label class="block text-xs text-gray-500 uppercase tracking-wider mb-1 font-bold">Бр.
                                                часови</label>
                                        ...
                                        <div class="col-span-3">
                                            <label
                                                class="block text-xs text-gray-500 uppercase tracking-wider mb-1 font-bold">Попуст
                                                (%)</label>
                                ...
                                @else
                                    <div class="grid grid-cols-5 gap-2 items-end">
                                        <div class="col-span-4 w-full relative" x-data="{ open: false }"
                                            ...
                                            <label
                                                class="block text-xs text-gray-500 uppercase tracking-wider mb-1 font-bold">Избери
                                                ученик</label>
                                            <div @click="open = !open"
                                                class="w-full h-9 border border-gray-200 rounded-xl shadow-sm text-xs font-medium bg-white flex items-center justify-between px-3 cursor-pointer">
                                                <span>
                                        ...
                                        <div>
                                            <label
                                                class="block text-xs text-gray-500 uppercase tracking-wider mb-1 font-bold whitespace-nowrap">Попуст
                                                (%)</label>
                                            <input type="number" min="0" max="100" wire:model="discount_percent"
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                                        <label class="block text-xs text-gray-500 uppercase tracking-wider mb-1 font-bold">Од
                                            датум</label>
                                        <input type="date" wire:model="date_from"
                                            class="w-full h-10 border-gray-200 rounded-xl shadow-sm text-sm font-medium">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-500 uppercase tracking-wider mb-1 font-bold">До
                                            датум</label>
                            ...
                            @else
                                <div class="space-y-3 animate-fadeIn">
                                    <div>
                                        <label
                                            class="block text-xs text-gray-500 uppercase tracking-wider mb-1 font-bold">Клиент /
                                            Фирма</label>
                                        <input type="text" wire:model="service_client_name" placeholder="Внесете име..."
                                            class="w-full h-10 border-gray-200 rounded-xl shadow-sm text-sm text-blue-800 font-bold">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-500 uppercase tracking-wider mb-1 font-bold">Опис
                                            на услуга</label>
                                    ...
                                    <div class="grid grid-cols-12 gap-2 items-end">
                                        <div class="col-span-9">
                                            <label
                                                class="block text-xs text-gray-500 uppercase tracking-wider mb-1 font-bold">Вкупен
                                                износ</label>
                                            <div class="relative">
                                                ...
                                                <span class="absolute right-4 top-1 text-gray-400 font-bold">ДЕН</span>
                                            </div>
                                        </div>
                                        <div class="col-span-3">
                                            <label
...
<script>
    function confirmSendInvoiceEmail(id, number, sentAt) {
        ...
        Swal.fire({
            title: 'Фактурата е веќе испратена',
            html: `Фактура бр. <b>${number}</b> е испратена на <b>${sentAt}</b>.<br><br>Дали сакате повторно да ја пратите?`,
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#2563eb',
            cancelButtonColor: '#6b7280',
            confirmButtonText: 'Да, прати повторно',
    ...
    function confirmAction(type, id, number) {
        if (type === 'cancel') {
            Swal.fire({
                title: 'Поништи фактура?',
                html: `Поништување се користи само кога фактурата е креирана по грешка.<br><br><b>Фактура бр. ${number}</b>`,
                icon: 'warning',
                input: 'textarea',
                inputLabel: 'Причина за поништување',
                inputPlaceholder: 'Пр. Погрешен клиент, погрешен период, дупликат фактура...',
                inputAttributes: {
                    'aria-label': 'Причина за поништување',
                    maxlength: 500
                },
                inputValidator: (value) => {
                    if (!value || value.trim().length < 10) {
                        return 'Внесете причина со минимум 10 карактери.';
                    }
                },
                showCancelButton: true,
                confirmButtonColor: '#ef4444',
                cancelButtonColor: '#2563eb',
                confirmButtonText: 'Да, поништи ја',
                cancelButtonText: 'Откажи',
                reverseButtons: true,
                backdrop: `rgba(15, 23, 42, 0.5)`
            }).then((result) => {
        ...
        if (type === 'restore') {
            Swal.fire({
                title: 'Врати фактура?',
                html: `Дали сакате да ја вратите фактура бр. <b>${number}</b> во статус <b>Неплатена</b>?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#059669',
                cancelButtonColor: '#2563eb',
                confirmButtonText: 'Да, врати ја',
                cancelButtonText: 'Откажи',
                reverseButtons: true,
                backdrop: `rgba(15, 23, 42, 0.5)`
            }).then((result) => {
        ...
        Swal.fire({
            title: 'Целосно бришење?',
            html: `За трајно бришење на фактура бр. <b>${number}</b>, внесете го точниот број на фактура.<br><br>Ова не може да се врати!`,
            icon: 'warning',
            input: 'text',
            inputPlaceholder: 'Внеси број на фактура',
            inputAttributes: {
                autocapitalize: 'off',
                autocorrect: 'off'
            },
            inputValidator: (value) => {
                if (!value || value.trim() !== number) {
                    return 'Внесете го точниот број на фактура за потврда.';
                }
            },
            showCancelButton: true,
            confirmButtonColor: '#dc2626',
            cancelButtonColor: '#2563eb',
            confirmButtonText: 'Да, избриши ја!',
            cancelButtonText: 'Откажи',
            reverseButtons: true,
            backdrop: `rgba(15, 23, 42, 0.5)`
        }).then((result) => {
