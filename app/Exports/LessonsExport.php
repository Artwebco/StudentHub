...
{
    protected $search, $type, $status, $from, $to;
    protected $totalSum = 0;

    public function __construct($search, $type, $status, $from, $to)
    {
        $this->search = $search;
        $this->type = $type;
        $this->status = $status;
    ...
    public function query()
    {
        ...
        if ($this->status) {
            $query->where('lesson_status', $this->status);
        }
    ...
    {
        $timeRange = ($lesson->start_time && $lesson->end_time)
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->setCellValue("E{$lastRow}", 'ВКУПНО:');
                $event->sheet->setCellValue("F{$lastRow}", $this->totalSum);
                ...
                $event->sheet->getStyle("F{$lastRow}")->getAlignment()->setHorizontal('right');
            },
        ];
    }
}
